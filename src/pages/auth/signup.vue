    ...
    export default class Signup extends wepy.page{
        ...
        methods= {
            async sendCheckCode(e){
                ...
                let result = await this.$parent.globalData.post(`${api.api.auth.verifyCode.url}`,{mobile: this.mobile})
                if(result.errorCode){
            ...
            async register(){
                ...
                let result = await this.$parent.globalData.post(`${api.server}/auth/wxa/user/register`,{fullname : this.data.fullname, mobile : this.data.mobile, checkcode : this.data.checkcode})
