...
export default class extends wepy.app {
    ...
    hasToken(){
        ...
        let expireTime = self.globalData.expireTime || wx.getStorageSync('user:expireTime');
    ...
    async tokenIsExpired(){
        ...
        let eptime = parseFloat(self.globalData.expireTime || wx.getStorageSync('user:expireTime'));
        console.log(">>>>> 本地 token 的失效时间 VS 对比 server 时间差值(分钟）",(eptime - now)/1000/60)
        if((eptime - now)<=self.globalData.tokenLose){//如果失效差值在规定范围之内，则重新刷取 token
            return true;
        }else{
            return false;
        }
